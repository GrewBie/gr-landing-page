...
const FAQ_ITEMS = [
  ...
  {
    q: "What is Forca by Grewbie?",
    a: "Forca is an AI product built by Grewbie that automates back-office work for Chartered Accountant (CA) firms in India — automatically chasing clients for missing documents over WhatsApp, running deterministic (rule-based, auditable) GST reconciliation against your books, and routing every action through a human-in-the-loop approval gate before it's finalized. Forca is currently in a live paid pilot with a CA firm client.",
  },
  {
...
export default function LandingClient() {
  const [modal, setModal] = useState<{ open: boolean; interest: "demo" | "brand-cure" | "forca" | "general"; title: string; subtitle: string }>({
    open: false,
    interest: "demo",
  ...
  const openForca = () =>
    setModal({ open: true, interest: "forca", title: "Get early access to Forca", subtitle: "Tell us about your CA practice and we'll set up a walkthrough." });
  ...
  return (
    <div className="relative bg-black text-white overflow-x-hidden">
      ...
      <main>
        ...
        {/* ── Forca ── */}
        <section id="forca" aria-labelledby="forca-heading" className="relative z-10 py-24 border-t border-white/5">
          <div className="max-w-6xl mx-auto px-6">
            <div className="text-center mb-14">
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 text-emerald-400 text-xs font-medium mb-6">
                <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" aria-hidden />
                Live paid pilot — onboarding CA firms
              </div>
              <p className="text-sm font-semibold tracking-widest uppercase text-emerald-500 mb-4">Also from Grewbie</p>
              <h2 id="forca-heading" className="text-3xl md:text-5xl font-bold text-white mb-4">
                Forca
              </h2>
              <p className="text-zinc-400 text-lg max-w-2xl mx-auto">
                AI that runs your CA practice&rsquo;s back office. Stop chasing clients for documents. Stop manually
                reconciling GST. Let AI handle the repetitive work your practice runs on.
              </p>
            </div>
            ...
            <div className="max-w-3xl mx-auto text-center mb-14">
              <p className="text-zinc-400 leading-relaxed">
                Chartered Accountant firms lose hours every week on tasks that don&rsquo;t need a human — chasing
                clients over WhatsApp for missing documents, manually cross-checking GST filings against books, and
                tracking approvals across spreadsheets and chat threads. Forca automates this layer without removing
                human control over what matters.
              </p>
            </div>

            {/* What Forca does */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-14" role="list">
              {[
                {
                  title: "WhatsApp Document Chasing",
                  desc: "Forca automatically follows up with clients over WhatsApp, requesting missing documents, sending reminders, and tracking what's been received — no more manual chasing.",
                  icon: (
                    <svg className="w-6 h-6 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} aria-hidden>
                ...
                {
                  title: "Deterministic GST Reconciliation",
                  desc: "Forca reconciles GST data against your books using rule-based, deterministic logic — not black-box guessing — so every match is explainable and auditable.",
                  icon: (
                    <svg className="w-6 h-6 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} aria-hidden>
                ...
                },
                {
                  ...
                  desc: "AI handles the repetitive work, but nothing gets finalized without your sign-off. Every reconciliation and client communication passes through an approval checkpoint before it's actioned.",
                  icon: (
                    <svg className="w-6 h-6 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} aria-hidden>
                ...
                {
                  title: "Append-Only Action Ledger",
                  desc: "Every action Forca takes — every message sent, every reconciliation performed — is logged in an immutable, auditable trail. Full transparency, always traceable.",
                  icon: (
                    <svg className="w-6 h-6 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} aria-hidden>
                ...
                <article
                  key={card.title}
                  className="p-6 rounded-2xl bg-zinc-900/60 border border-white/5 hover:border-emerald-500/20 transition-all duration-300 group"
                  role="listitem"
                >
                  <div className="w-10 h-10 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center mb-4 group-hover:bg-emerald-500/20 transition-colors" aria-hidden>
                    {card.icon}
            ...
            {/* How it works + Why Forca */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-4">
              <div className="rounded-2xl bg-zinc-900/40 border border-white/5 p-6 md:p-8">
                <h3 className="text-white font-bold text-lg mb-3">How it works</h3>
                <p className="text-zinc-400 text-sm leading-relaxed">
                  Built on a multi-agent orchestration architecture, Forca&rsquo;s agents work together: one manages
                  client communication, one handles document parsing, one performs reconciliation — all coordinated
                  through a central workflow engine, with a mobile app for practitioners to review and approve in
                  real time.
                </p>
              </div>
              <div className="rounded-2xl bg-zinc-900/40 border border-white/5 p-6 md:p-8">
                <h3 className="text-white font-bold text-lg mb-3">Why Forca</h3>
                <ul className="space-y-2.5 text-sm text-zinc-400" role="list">
                  {[
                    "Built for Indian CA firms — not a generic global tool retrofitted for local compliance quirks.",
                    "Deterministic, not probabilistic, for compliance-critical work — GST reconciliation logic is rule-based and auditable, not an LLM guess.",
                    "Currently in a live paid pilot with a CA firm client, proving real-world workflow fit.",
                  ].map((point) => (
                    <li key={point} className="flex items-start gap-2.5" role="listitem">
                      <svg className="w-4 h-4 text-emerald-400 mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden>
            ...
            <div className="text-center mt-10">
              <button
                onClick={openForca}
                className="inline-block px-8 py-3 rounded-xl border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/10 font-semibold text-sm transition-all duration-200 cursor-pointer"
                aria-label="Get early access to Forca for your CA practice"
              >
                Get early access to Forca →
              </button>
            </div>
          </div>
        </section>
              ...
              <div className="grid md:grid-cols-2 gap-8 text-zinc-400 text-base leading-relaxed">
                ...
                <div>
                  <p className="mb-4">
                    ...
                    and globally. <strong className="text-white">Forca</strong>, currently in a live paid pilot,
                    automates back-office work for Chartered Accountant firms — WhatsApp document chasing,
                    deterministic GST reconciliation, and human-in-the-loop approvals.
                  </p>
